...
Após instalar `vue-apollo` em seu app, todos seus componentes poderão utilizar Apollo através da opção especial `apollo`.

## opções `apollo`

Para declarar consultas apollo em seu componente vue, adicione o objeto `apollo` à opção de componente:

```js
new Vue({
  apollo: {
    // Opções específicas Apollo
  },
})
```
